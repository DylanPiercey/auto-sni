...
var LE_SNI = require('le-sni-auto')
...
function createServer (opts, handler) {
  ...
  var le = LE.create({
    ...
    sni: LE_SNI.create({
      ...
      renewBy: EXPIRE_TIME,
      getCertificatesAsync: getCertificatesAsync
    }),
    store: LE_STORE.create({
      configDif: '~/letsencrypt/etc',
      debug: opts.debug
    }),
    challenges: { 'http-01': LE_CHALLENGE.create({
  ...
  // Pull out sni options.
  var httpsServerOptions = le.httpsOptions
  ...
  httpsServer
    ...
    .listen(ports.https || 0, opts.ip)

  // Start the http -> https server.
  httpServer
    .on('error', httpsServer.emit.bind(httpsServer, 'error'))
    .listen(ports.http || 0, opts.ip)
    .unref()
   ...
   * @return {Promise}
   */
  function getCertificatesAsync (hostname) {
    var bundle = findBundle(hostname)
    if (!bundle) return Promise.reject(new Error('AutoSNI: Unlisted domain requested.'))
    return le.check({ domains: bundle }).then(function handleResults (results) {
