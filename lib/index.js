...
function createServer (opts, handler) {
	...
	for (var key in opts.subject) {
		if (!(key in SUBJ)) {
			throw new TypeError("AutoSNI: Invalid subject option '" + key + "'.");
		}

		if (key === "country" && opts.subject[key].length > 3) {
			throw new TypeError(
				"AutoSNI: Invalid subject.country option '" + opts.subject[key] + "'. " +
				"Country must be a 2-3 letter code."
			)
		}
	}
	...
	return Promise.all([
		// Ensure that the requested ports are available.
