...
function load (domains, opts) {
  ...
  var configDir = path.join(home, '/letsencrypt/etc')
    ...
    .catch(function registerOrRenewCert () {
      return mkdirp(liveDir)
        .then(function createFallBackCert () {
          ...
          return cp.exec(
            getKeyGenCmd(hostname, opts, curOpts),
...
function getKeyGenCmd(hostname, opts, curOpts) {
  
  // Create the private key and a self signed cert.
  return 'openssl req -x509 -newkey rsa:2048 -keyout privkey.pem -out cert.pem -days 90 -nodes -subj "' +
    '/CN=' + hostname +
    '/emailAddress=' + opts.email +
  '"' +
  // Create a chain for the self signed cert.
  ' && ' + ((process.platform == 'win32') ? 'type' : 'cat') + ' privkey.pem cert.pem | tee -a chain.pem fullchain.pem' +
