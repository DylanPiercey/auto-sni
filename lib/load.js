	...
	return lock[hostname] = Promise.all([
		...
		.catch(function createNewCert () {
			return mkdirp(liveDir)
				.then(function () {
					console.log(createSubj(hostname, opts.subject));
					...
					return cp.exec(
						...
						createSubj(hostname, opts.subject) +
...
/**
 * Utility to create a valid CSR Subject string.
 *
 * @param {String} hostname - the hostname/common name.
 * @param {Object} subj
 */
function createSubj (hostname, subj) {
	var result = ["CN=" + hostname];
	var val;

	for (var key in subj) {
		result.push(SUBJ[key] + "=" + String(subj[key]).replace("/", "\\/"))
	};
	return "'/" + result.join("/") + "'";
}
