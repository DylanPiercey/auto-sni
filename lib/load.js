    ...
    .catch(function registerOrRenewCert () {
      return mkdirp(liveDir)
        .then(function createFallBackCert () {
          ...
          var privKey = fs.readFileSync(path.join(liveDir, 'privkey.pem'));
          var cert = fs.readFileSync(path.join(liveDir, 'cert.pem'));
          fs.writeFileSync(path.join(liveDir, 'chain.pem'), privKey + cert);
          fs.writeFileSync(path.join(liveDir, 'fullchain.pem'), privKey + cert);
          fs.writeFileSync(path.join(liveDir, 'opts.json'), curOpts);
          return 
        })
