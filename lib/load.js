...
function load (domains, opts) {
  ...
  var configDir = path.join(process.env.HOME || process.env.USERPROFILE, '/letsencrypt/etc')
  ...
  var hostLock = Promise.all([
    ...
    .then(function checkCertExpiryAndOptions (data) {
      ...
      if (prevOpts.domains !== String(opts.domains)) throw new Error('AutoSNI: Domain bundle changed.')
        ...
        .then(function createFallBackCert () {
          ...
          if (process.platform === 'win32') {
            return cp.exec(
              ...
              ' && type privkey.pem cert.pem >> chain.pem' +
              ' && type privkey.pem cert.pem >> fullchain.pem' +
